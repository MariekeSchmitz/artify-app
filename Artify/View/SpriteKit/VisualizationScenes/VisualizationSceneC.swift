...
class VisualizationSceneC: SKScene {
...
//        self.anchorPoint
        ...
        let startXPitchLines = getX(angle: angle, step: 0, radius: radius) + frame.midX
        let startYPitchLines = getY(angle: angle, step: 0, radius: radius) + frame.midY
        
        let startXTimbreLines = getX(angle: angle, step: 0, radius: timbreRadius) + frame.midX
        let startYTimbreLines = getY(angle: angle, step: 0, radius: timbreRadius) + frame.midY
...
//            var circle = drawCircle(radius: 2, posX: getX(angle: angle, step: i, radius: radius/2) + frame.midX , posY: getY(angle: angle, step: i, radius: radius/2) + frame.midY, fillColor: values[i].beatConfidence > 0.8 ? SKColor.blue : SKColor.white)
//            self.addChild(circle)
            
            if(values[i].barChange) {
                var circle = drawCircle(radius: 2, posX: getX(angle: angle, step: i, radius: radius*9/20) + frame.midX , posY: getY(angle: angle, step: i, radius: radius*9/20) + frame.midY, fillColor: SKColor.white)
                self.addChild(circle)
            }
//            
            if(values[i].sectionChange) {
                var circle = drawCircle(radius: 30, posX: getX(angle: angle, step: i, radius: radius*9/20) + frame.midX , posY: getY(angle: angle, step: i, radius: radius*9/20) + frame.midY, fillColor: SKColor.blue.withAlphaComponent(0.4))
                self.addChild(circle)
            }
